...
@section('content')
    ...
    <div class="container my-5">
        <form action="{{ route('blog.update', $blog->id) }}" method="POST" enctype="multipart/form-data" class="col-8 mx-auto">
            ...
            <div class="mb-3">
                ...
                <select class="form-select @error('kategori') is-invalid @enderror" aria-label="kategori" id="kategori" name="kategori">
                    @foreach ($kategori as $item)
                        <option value="{{ $item->id }}" {{ $blog->id_kategori == $item->id? "selected" : "" }}>{{ $item->nama }}</option>
                    @endforeach
                </select>
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control @error('konten') is-invalid @enderror" id="konten" rows="3" name="konten">{{ old('konten', $blog->konten) }}</textarea>
